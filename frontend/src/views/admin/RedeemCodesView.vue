...
async function load() {
  loading.value = true
  try {
    const data = await api.get(`/api/admin/redeem-codes?page=${page.value}&size=15`)
...
async function save() {
  try {
    if (editing.value) {
      await api.patch(`/api/admin/redeem-codes/${editing.value.id}`, {
        note: form.note,
...
async function toggle(row) {
  await api.patch(`/api/admin/redeem-codes/${row.id}`, { enabled: !row.enabled })
  load()
}
...
async function remove(row) {
  await api.del(`/api/admin/redeem-codes/${row.id}`)
...
<template>
  <div class="page">
    ...
    <el-alert type="info" :closable="false" style="margin-bottom: 16px">
      兑换码仅供已注册用户兑换额度（个人中心 → 兑换码）；注册邀请码在「邀请码」页管理，两者互不影响。
    </el-alert>
    ...
    <el-card>
      <el-table :data="items" v-loading="loading">
        <el-table-column prop="code" label="兑换码" min-width="140">
          <template #default="{ row }">
            <code>{{ row.code }}</code>
            <el-button size="small" text type="primary" @click="copy(row)">复制</el-button>
          </template>
        </el-table-column>
        <el-table-column label="兑换额度" min-width="230">
          <template #default="{ row }">
            <span class="text-muted" style="font-size: 12px">{{ quotaSummary(row.quotas) }}</span>
            <div v-if="row.pool_type === 'temporary'" style="font-size: 12px; color: #b88230">限时组 · {{ row.fixed_expires_at ? `${fmtTime(row.fixed_expires_at)} 固定到期` : `${row.valid_days}天+${row.valid_hours}时` }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="170" fixed="right">
          <template #default="{ row }">
            ...
            <el-popconfirm title="确定删除该兑换码？" @confirm="remove(row)">
              <template #reference>
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑兑换码' : '创建兑换码'" width="min(480px, 94vw)">
      <el-form label-width="110px">
        <el-form-item v-if="!editing" label="兑换码">
        ...
        <el-divider content-position="left">兑换额度</el-divider>
        <el-form-item v-for="f in FEATURES" :key="f" :label="FEATURE_LABELS[f]">
          <el-input-number v-model="form.quotas[f]" :min="0" :max="999999" />
        </el-form-item>
