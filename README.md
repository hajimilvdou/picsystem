...
> 上游能力来自 [chatgpt2api](https://github.com/yukkcat/chatgpt2api)（对话 / 联网搜索 / 文生图 / 图生图 / 局部编辑 / PPT / PSD）。
...
| 角色 | 功能 |
| :--- | :--- |
| 普通用户 | 邀请码注册（自定义或随机生成凭证，支持注册审批制）· 首次登录滚动阅读并同意免责协议 · 对话（SSE 流式 / 多会话 / Markdown / 代码高亮 / 上游标记自动过滤）· 文生图 / 图生图（1-4 张参考图）/ 图片编辑（局部重绘）· 联网搜索（引用来源）· PPT / PSD 任务（进度轮询 / 下载）· 我的文件（图库 / 标签筛选 / 缩略图 / 多选打包下载 / 批量删除 / 图片压缩 / 一键带去编辑）· 个人 API 密钥（OpenAI 兼容 `/v1`）· 每日签到与兑换码领额度 · 双池额度（限时优先扣减）· 额度与调用记录查询 · 登录设备管理（陌生设备一键注销） |
...
- **存储治理**：每用户存储配额（全局默认 + 个人覆盖）、产物保留时长（如 24 小时自动删除）、日志保留策略与孤儿文件清理（含缩略图残留兜底），后台每小时自动执行
- **图库缩略图**：图片按长边 400px 生成 JPEG 缩略图，首次访问时生成并落盘缓存（存量图片无需回填），图库/绘图页按缩略图加载、点开看原图；原图删除时缩略图一并清理
- **图片压缩**：把 PNG / JPEG / WebP 重新编码为 WebP 以省存储（图库多选 → 压缩图片，可选质量档）。**不可逆**：确认前先干跑预估体积收益，确认后才替换原文件；压缩后反而更大的会自动跳过。GIF（会丢动画）与 PPT / PSD / PDF 不处理
...
| `POST /v1/images/edits` | 图生图 / 局部编辑（multipart，可选 `mask`） | n 次/次 |
